document.addEventListener('DOMContentLoaded', () => {
  ...
  const calcolaEta = d => { if (!d) return ''; const o = new Date(), n = new Date(d); let e = o.getFullYear() - n.getFullYear(); const m = o.getMonth() - n.getMonth(); if (m < 0 || (m === 0 && o.getDate() < n.getDate())) e--; return e; };
  ...
  function mostraAtleti(){ // Con filtro attivo
  ...
  document.querySelector('#dashboard').addEventListener('click', e => {
    ...
    if(!card) return;
    filtroAttivo = card.dataset.filter || 'all';
    mostraAtleti();
  });

  function cancella(id){const a=caricaAtleti(),i=a.findIndex(x=>x.id===id);if(i>-1&&confirm('Confermi eliminazione?')){a.splice(i,1);salvaAtleti(a);mostraAtleti();}}
