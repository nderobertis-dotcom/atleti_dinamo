const DOTAZIONE_ABBIGLIAMENTO = [
  "MAGLIA GARA","PANTALONCINO GARA","TUTA DI RAPPRESENTANZA","TUTA LAVORO",
  "POLO","TSHIRT ALLENAMENTO","PANTALONCINO ALLENAMENTO","BERMUDA",
  "GIUBBOTTO","MAGLIA PRE-GARA","CALZA BIANCA"
];
...
function percentualeCompletamento() {
  ...
  const campi = [
    ...
    'provResidenza','svm'
    // se vuoi includere le taglie abbigliamento nel calcolo di completamento, aggiungi i campi qui
  ];
  ...
  div.innerHTML = `
    <div class="atleta">
      ...
      <div class="atleta-dettagli">
        ...
        <button class="btn-backup" onclick="document.getElementById('showAbbigliamento${i}').style.display=(document.getElementById('showAbbigliamento${i}').style.display==='none')?'block':'none';">ABBIGLIAMENTO</button>
        <div id="showAbbigliamento${i}" style="display:none; margin:12px 0;">
          ${DOTAZIONE_ABBIGLIAMENTO.map(nome =>
            `<div>${nome}: <span style="font-weight:600">${a.abbigliamento && a.abbigliamento[nome]? a.abbigliamento[nome] : '-'}</span></div>`).join('')}
        </div>
      </div>
...
function modificaAtleta(i) {
  ...
  div.innerHTML = `
    <div class="atleta" style="background: #f9f9f9; border-color: #4CAF50;">
      ...
      <button type="button" id="btn-abbigliamento-edit" style="margin:12px 0;">ABBIGLIAMENTO</button>
      <div id="section-abbigliamento-edit" style="display:none; margin:10px 0 0 0; padding:10px; background:#f4f7ff; border-radius:7px; border:1px solid #d3d8f0;">
        ${DOTAZIONE_ABBIGLIAMENTO.map(nome=>
          `<div style="margin-bottom:9px;">
            <label style="display:inline-block; min-width:170px;">${nome}</label>
            <select class="select-card" id="taglia_${nome.replace(/\s+/g,'_')}">
              <option value="">Seleziona</option>
              ${TAGLIE.map(tg=>`<option value="${tg}"${(a.abbigliamento && a.abbigliamento[nome]===tg)?' selected':''}>${tg}</option>`).join('')}
            </select>
          </div>`
        ).join('')}
      </div>
  ...
  document.getElementById('btn-abbigliamento-edit').onclick = function(){
    const box = document.getElementById('section-abbigliamento-edit');
...
function mostraCardAggiungi() {
  ...
  div.innerHTML = `
    <div class="atleta" style="background: #f9f9f9; border-color: #4CAF50;">
      ...
      <button type="button" id="btn-abbigliamento" style="margin:12px 0;">ABBIGLIAMENTO</button>
      <div id="section-abbigliamento" style="display:none; margin:10px 0 0 0; padding:10px; background:#f4f7ff; border-radius:7px; border:1px solid #d3d8f0;">
        ${DOTAZIONE_ABBIGLIAMENTO.map(nome=>
          `<div style="margin-bottom:9px;">
            <label style="display:inline-block; min-width:170px;">${nome}</label>
            <select class="select-card" id="taglia_${nome.replace(/\s+/g,'_')}">
              <option value="">Seleziona</option>
              ${TAGLIE.map(tg=>`<option value="${tg}">${tg}</option>`).join('')}
            </select>
          </div>`
        ).join('')}
      </div>
  ...
  document.getElementById('btn-abbigliamento').onclick = function(){
    const box = document.getElementById('section-abbigliamento');
    box.style.display = (box.style.display==='none') ? 'block':'none';
  };
  applicaUpperCaseInput();
}
...
function salvaNuovoAtleta() {
  ...
  let abbigliamento = {};
  DOTAZIONE_ABBIGLIAMENTO.forEach(nome => {
    abbigliamento[nome] = document.getElementById('taglia_'+nome.replace(/\s+/g,'_')).value;
  });
  let atleta = {
