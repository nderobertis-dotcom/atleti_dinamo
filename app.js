document.addEventListener("DOMContentLoaded", () => {
  ...
  const statoVisita = (s) => {
    ...
    return diff < 0 ? "data-scaduta" : diff <= 31 ? "data-scanza" : "data-ok";
  };
  ...
  function mostraAtleti(filtro = filtroAttivo) {
    ...
    a.forEach((x) => {
      ...
      const li = document.createElement("li");
      li.innerHTML = `<span class='list-info'>
            <strong>${x.codiceAtleta}</strong> - ${x.nome} ${x.cognome} – ${x.sesso} – ${x.ruolo}
            <div><span>nato il ${formattaData(x.dataNascita)}</span>
            <span>eta: ${calcolaEta(x.dataNascita)}</span>
            <span>CF: ${x.codiceFiscale}</span>
            <span>Cell: ${x.cellulare}</span></div>
  ...
  function visualizza(id) {
    ...
    document.getElementById(
      "dettaglio-atleta"
    ).innerHTML = `<h2>${a.codiceAtleta} - ${a.nome} ${a.cognome}</h2>
  ...
  function modifica(id) {
    ...
    ["codiceAtleta", "nome", "cognome", "sesso", "dataNascita", "ruolo", "codiceFiscale", "cellulare", "scadenzaVisita"].forEach(
      (f) => {
        document.getElementById("mod-" + f).value = atleta[f] || "";
      }
    );
  ...
  document.getElementById("modifica-form").onsubmit = async (e) => {
    ...
    [
      ...
      lista[idx][f] = document.getElementById("mod-" + f).value.trim();
      if (f !== "sesso") lista[idx][f] = lista[idx][f].toUpperCase();
    });
  ...
  document.getElementById("print-btn").onclick = () => {
    ...
    let html = `<h1>Lista Atleti</h1>
      <table border="1" cellspacing="0" cellpadding="6">
      <thead><tr>
        <th>Codice</th><th>Nome</th><th>Cognome</th><th>Ruolo</th><th>Data Nascita</th><th>Scadenza Visita</th>
      </tr></thead><tbody>`;
    lista.forEach((x) => {
      html += `<tr>
        <td>${x.codiceAtleta}</td><td>${x.nome}</td><td>${x.cognome}</td><td>${x.ruolo}</td>
        <td>${formattaData(x.dataNascita)}</td><td>${formattaData(x.scadenzaVisita)}</td>
      </tr>`;
    });
    html += "</tbody></table>";
